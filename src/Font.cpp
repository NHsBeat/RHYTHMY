...
// 8x8 bitmap font, ASCII 32-126 (95 chars)
// Each row byte: bit 0 = leftmost pixel
static const uint8_t GLYPH[95][8] = {
...
void Font::drawChar(SDL_Renderer* r, int x, int y, char c, SDL_Color col, int scale) {
    int idx = (unsigned char)c - 32;
    if (idx < 0 || idx >= 95) return;
    const uint8_t* g = GLYPH[idx];
...
void Font::drawCharCentered(SDL_Renderer* r, int cx, int cy, char c, SDL_Color col, int scale) {
    int idx = (unsigned char)c - 32;
    if (idx < 0 || idx >= 95) return;
    const uint8_t* g = GLYPH[idx];
    // Ink bounding box of the glyph
    int x0 = 8, x1 = -1, y0 = 8, y1 = -1;
    for (int row = 0; row < 8; row++)
    ...
    if (x1 < 0) return;  // blank glyph (e.g. space)
    // Place the ink centre on (cx,cy), rounded to the nearest pixel
    int drawX = cx - (int)((x0 + x1 + 1) * 0.5f * scale + 0.5f);
    int drawY = cy - (int)((y0 + y1 + 1) * 0.5f * scale + 0.5f);
    SDL_SetRenderDrawColor(r, col.r, col.g, col.b, col.a);
    for (int row = 0; row < 8; row++) {
        uint8_t bits = g[row];
        for (int colb = 0; colb < 8; colb++)
            if (bits & (1 << colb)) {
                SDL_Rect rc{drawX + colb * scale, drawY + row * scale, scale, scale};
                SDL_RenderFillRect(r, &rc);
            }
    }
}

void Font::drawText(SDL_Renderer* r, int x, int y, std::string_view s, SDL_Color col, int scale) {
    int cx = x;
    for (char c : s) {
        drawChar(r, cx, y, c, col, scale);
        cx += 8 * scale;
    }
}

void Font::drawTextRight(SDL_Renderer* r, int rx, int y, std::string_view s, SDL_Color col, int scale) {
    int w = textW(s, scale);
    drawText(r, rx - w, y, s, col, scale);
}

void Font::drawTextCenter(SDL_Renderer* r, int cx, int y, std::string_view s, SDL_Color col, int scale) {
    int w = textW(s, scale);
    drawText(r, cx - w / 2, y, s, col, scale);
}
